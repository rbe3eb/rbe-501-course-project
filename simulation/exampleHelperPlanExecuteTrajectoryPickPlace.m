...
%   checkCollision (via the collisionHelper function) to check for
...
numBodies = numel(nonzeros(collisionHelper.PartMask));
% 
...
% 
% targetStates = [positions(:,1:timestep+1);velocities(:,1:timestep+1);accelerations(:,1:timestep+1)]'; 
% targetTime = timestamp(1:timestep+1);
% initState = [positions(:,1);velocities(:,1)]';
% trajTimes = targetTime(1):coordinator.TimeStep:targetTime(timestep+1);
% 
...
function cineq = myIneqConFunction(X,U,e,data, collisionHelper, safetyDistance, world)
...
function [G,Gmv,Ge] = nlmpcJacobianConstraint(X,U,e,data, world, robot, collisionHelper)
    ...
    for i=1:p
        collisionConfig = X(i+1,1:numJoints);
        collisionHelper.ExhaustiveChecking = true;
        [~ ,allWntPts,allBodyNames] = checkRobotWorldCollision(collisionHelper, collisionConfig, world);
        for j=1:numBodies
            for k=1:numObstacles
                ...
                bodyJacobian = geometricJacobian(robot,collisionConfig', allBodyNames{j});
